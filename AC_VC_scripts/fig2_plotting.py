...
from AC_VC_scripts.MAPseq_data_processing import FitResult
...
def plot_projection_probability_by_area_combined_model(
    ax,
    area,
    proj_by_ap_pos_df,
    model,
    ...
    dist_vals = df_area["distance"].values
    ...
    for i in range(num_bins):
        ...
        if np.sum(in_bin) > 0:
            ...
            bin_mean_dist.append(dist_vals[in_bin].mean())
        else:
            bin_proportions.append(np.nan)
            bin_mean_dist.append(np.nan)
    bin_proportions = np.array(bin_proportions)
    bin_mean_dist = np.array(bin_mean_dist)
    ...
    ap_grid = bin_centers[valid]
    new_data = pd.DataFrame(
        {
            "AP_position": ap_grid,
            "distance": bin_mean_dist[valid],
            "Area": [area] * len(ap_grid),
        }
    )
    new_data["Area"] = new_data["Area"].astype(model.model.data.frame["Area"].dtype)
    pred = model.get_prediction(new_data)
    pred_df = pred.summary_frame(alpha=0.05)
    if "mean" in pred_df.columns:
        to_plot = "mean"
    else:
        to_plot = "predicted"  # the column name for predictions depends on which version of statsmodels you have
    ax.plot(
        ap_grid, pred_df[to_plot], "-", color=fit_color, linewidth=1, label="Logit fit"
    )
    ...
    if include_pval:
        if not supplementary_fig:
            y_title_name = "Projection probability"
        else:
            y_title_name = ""
        ff.myPlotSettings_splitAxis(
            fig=ax.figure,
...
def plot_area_projection_probs(
    ...
    fig,
    axes,
    include_dist_as_covariate=True,
    ...
    pval_df, proj_by_ap_pos_df, combined_model = mdp.pooled_area_probabilities(
